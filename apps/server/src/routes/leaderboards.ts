...
export function leaderboardRoutes(app: FastifyInstance): void {
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const entries = await redis.zrevrange(
        REDIS_KEYS.weeklyLeaderboard('time'),
        start,
        end,
        'WITHSCORES'
      );
  ...
  app.get(
    ...
    async (request: FastifyRequest, reply: FastifyReply) => {
      ...
      const entries = await redis.zrevrange(
        REDIS_KEYS.weeklyLeaderboard('commits'),
        start,
        end,
        'WITHSCORES'
      );
  ...
  /**
   * GET /leaderboards/team/:teamId - Team-scoped leaderboard
   */
  app.get(
    '/team/:teamId',
    { onRequest: [app.authenticate] },
    async (request: FastifyRequest, reply: FastifyReply) => {
      const { userId } = request.user as { userId: string };
      const paramsSchema = z.object({ teamId: z.string().min(1) });
      const paramsResult = paramsSchema.safeParse(request.params);

      if (!paramsResult.success) {
        return reply
          .status(400)
          .send({ error: { code: 'INVALID_PARAMS', message: 'Invalid team ID' } });
      }
      ...
      if (!isOwner) {
        ...
        if (!membership) {
          return reply
            .status(403)
            .send({ error: { code: 'FORBIDDEN', message: 'Not a team member' } });
        }
      }
      ...
      const memberIds = [...members.map((m) => m.userId), team.ownerId];
      const uniqueIds = [...new Set(memberIds)];

      // Get scores for all members from Redis
      const pipeline = redis.pipeline();
      ...
      // Build score map
      const scoreEntries: { userId: string; score: number }[] = [];
      for (let i = 0; i < uniqueIds.length; i++) {
        const id = uniqueIds[i];
        const result = results?.[i];
        const score = result?.[1] ? parseInt(result[1] as string, 10) : 0;
        if (id) {
          scoreEntries.push({ userId: id, score });
        }
      }
      ...
      const leaderboard = scoreEntries.map((s, index) => {
        const user = userMap.get(s.userId);
        return {
          rank: index + 1,
          userId: s.userId,
